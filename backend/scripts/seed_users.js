...
async function seedUsers() {
    try {
        ...
        for (let i = 0; i < 50; i++) {
            const name = names[i % names.length] + (i > 19 ? ` ${i}` : ''); // Ensure unique names if > 20
            const dept = departments[Math.floor(Math.random() * departments.length)];
            const year = years[Math.floor(Math.random() * years.length)];
            const collegeId = `CS2025${String(i + 1).padStart(3, '0')}`;
            // 20% users are already claimed/verified
            ...
            users.push({
                ...
                collegeId: collegeId,
                password: isClaimed ? 'password123' : undefined, // Password only for claimed
                department: dept,
                year: year,
                isClaimed: isClaimed,
                verified: isClaimed, // If claimed, let's say they verified (simplify for demo)
        ...
        console.log(`College ID: ${sample.collegeId}, Name: ${sample.name}`);
